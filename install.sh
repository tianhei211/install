...
ensure_qrencode() {
  ...
  if command -v apt >/dev/null 2>&1; then
    if ! DEBIAN_FRONTEND=noninteractive apt install -y qrencode >>"$qr_log" 2>&1; then
      apt update >>"$qr_log" 2>&1 || sync_system_time
      DEBIAN_FRONTEND=noninteractive apt install -y qrencode >>"$qr_log" 2>&1 || return 1
    fi
  fi
  ok "二维码工具安装完成。"
}
...
svc_restart() {
  if command -v systemctl >/dev/null 2>&1; then
    ...
    sleep 1
    systemctl is-active --quiet sing-box || sleep 2
...
EOF
  ...
  ' "$CONF_DIR"/*.json > "$WORK_DIR/config.json" 2>/dev/null
}
...
change_user_cred() {
  echo "选择要修改凭据的协议："
  echo "1) VLESS（Reality / WS 统一 UUID）"
...
show_generated_links() {
  ...
  if [ -f "$f1" ]; then
    ...
    echo
    command -v qrencode >/dev/null 2>&1 && qrencode -t ANSIUTF8 -m 1 -s 1 "$link"
    echo
  fi
  ...
  [ "$found_any" = false ] && warn "未检测到任何已安装的协议配置。"
}

main_menu() {
  clear
  echo -e "${GREEN}===== Sing-box 纯净一键安装管理脚本 =====${RESET}"
  echo
  echo "1) 安装 VLESS + TCP + Reality (推荐)"
  echo "2) 安装 VMESS + WS"
